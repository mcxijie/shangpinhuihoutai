<template>
  <div class="app-container">
    ...
    <div style="margin-bottom: 20px">
      ...
      <el-button :disabled="selectedIds.length===0" type="danger" @click="revomveUsers"
      >批量删除
      </el-button>
    </div>
    ...
    <el-table
      ...
      <el-table-column align="center" label="操作" width="230">
        <template slot-scope="{row}">
          <HintButton icon="el-icon-user-solid" size="mini" title="分配角色" type="info"
                      @click="showAssignRole(row)"/>
          <HintButton icon="el-icon-edit" size="mini" title="修改用户" type="primary"
                      @click="showUpdateUser(row)"/>
          <el-popconfirm :title="`确定删除 ${row.username} 吗?`" @onConfirm="removeUser(row.id)">
            <HintButton slot="reference" icon="el-icon-delete" size="mini" style="margin-left:10px" title="删除用户"
                        type="danger"/>
          </el-popconfirm>
        </template>
      </el-table-column>
